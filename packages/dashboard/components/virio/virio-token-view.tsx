...
const protocolScenarios = [
  ["10k", "$9.0M", "180k", "$202.5k", "$30.4k", "30,375 / P"],
  ["100k", "$90.0M", "1.8M", "$2.025M", "$303.8k", "303,750 / P"],
  ["1M", "$900.0M", "18M", "$20.25M", "$3.038M", "3,037,500 / P"],
] as const;
...
export function VirioTokenView() {
  return (
    <main className="overflow-hidden bg-background pb-24 text-foreground">
      ...
      <section className="mx-auto max-w-6xl px-5 py-20 sm:px-8"><SectionHeading eyebrow="Protocol fees and VIRIO" title={<>Usage is measurable. <span className="text-virio-emerald">Price is market-determined.</span></>} description="Virio charges protocol fees in stablecoins such as USDC when recurring payments are successfully executed. At genesis, those fees remain in transparent protocol treasury accounting; fee-distribution and acquisition mechanisms are disabled." />
        <div className="mt-10 grid gap-4 md:grid-cols-3"><Card><p className="font-display text-xl font-semibold">1. Payment activity</p><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Customers authorize recurring USDC payments. Executors complete eligible charges and merchants receive stablecoin settlement. More successful usage means more observable protocol activity and fee records.</p></Card><Card><p className="font-display text-xl font-semibold">2. Protocol resources</p><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Fees support transparent treasury accounting, security operations and ecosystem work under approved controls. They do not automatically create a VIRIO price outcome.</p></Card><Card><p className="font-display text-xl font-semibold">3. Coordination utility</p><p className="mt-3 text-sm leading-relaxed text-muted-foreground">VIRIO is designed for defined roles: ecosystem incentives, future executor eligibility and security, merchant participation options, and governance. Each role must solve a real network need before it is activated.</p></Card></div>
        <Card><h3 className="font-semibold">Why VIRIO is not a decorative token</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Virio payments do not need VIRIO: that keeps stablecoin payment UX simple. VIRIO instead coordinates the people and services around those payments. Contributors can earn it for useful work; a future executor can stake it to signal accountability and compete for execution; merchants may opt into participation benefits; and governance can use it to make transparent network decisions. If a proposed role does not make the payment network safer, more useful or easier to grow, it should not be enabled.</p><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Protocol adoption can increase the relevance of these roles, but it does not guarantee demand, liquidity or price appreciation. VIRIO’s market price is established by independent market participants.</p></Card>
        <div className="mt-8"><h3 className="font-display text-2xl font-semibold">Protocol scenario math</h3><p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">Illustrative annual operating scenarios, not forecasts. Assumptions: average charge $50, 1.5 charges per active relationship each month, protocol fee of 0.25% of payment volume plus $1 per charge. The final two columns apply the currently proposed 15% Safety Module acquisition share only if it is security-approved and enabled.</p><div className="mt-5 overflow-x-auto rounded-2xl border border-border"><table className="w-full min-w-[850px] text-left text-sm"><thead className="bg-card text-muted-foreground"><tr><th className="p-4">Active relationships</th><th className="p-4">Annual payment volume</th><th className="p-4">Annual charges</th><th className="p-4">Protocol fees</th><th className="p-4">15% acquisition budget</th><th className="p-4">VIRIO acquired*</th></tr></thead><tbody>{protocolScenarios.map(([active, volume, charges, fees, budget, acquired]) => <tr key={active} className="border-t border-border bg-background"><td className="p-4 font-medium">{active}</td><td className="p-4">{volume}</td><td className="p-4">{charges}</td><td className="p-4">{fees}</td><td className="p-4 text-virio-emerald">{budget} USDC</td><td className="p-4 font-mono">{acquired}</td></tr>)}</tbody></table></div><p className="mt-3 text-xs leading-relaxed text-muted-foreground">*P is the actual average VIRIO execution price in USDC for that acquisition period. Example: a $303,750 budget acquires 303,750 ÷ P VIRIO before trading fees and slippage. This is a mechanical formula, not a price forecast, guarantee, or statement that the mechanism will be enabled.</p></div>
      </section>
